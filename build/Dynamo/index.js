...
var convertOperationToExpression = function (key, operation, value, value2) {
  ...
  if (operation === 'notContains') {
    return 'not contains('.concat(key, ', ').concat(value, ')');
  }
  if (operation === 'exist' || operation === 'exists') {
...
var DynamoWrapper = /** @class */ (function () {
  ...
  DynamoWrapper.prototype.buildQuery = function (pk, sk, gsiIndex) {
    ...
    var pkAttribute = (0, util_dynamodb_1.marshall)(
      { pk: pk },
              ...
              lastEvaluatedKeys.forEach(function (key) {
                ...
                if (
                  ((_a = modOptions.attributes) === null || _a === void 0
                    ? void 0
                    : _a.indexOf(''.concat(key))) === -1
                ) {
                  modOptions.attributes.push(''.concat(key));
                }
              });
            }
            lastKey =
  ...
  DynamoWrapper.prototype.scan = function (options, gsiName) {
    var _a;
    return __awaiter(this, void 0, void 0, function () {
      var buildOptions, input, response, formattedResponse;
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            buildOptions = this.processOptions(options);
            input = new client_dynamodb_1.ScanCommand(
              __assign(__assign({}, buildOptions), {
                TableName: this.model.tableName,
    ...
    if (options && options.filters && options.filters.length > 0) {
      ...
      options.filters.forEach(function (filter, index) {
        ...
        if (
          ...
          var key_1 = '#'.concat(index);
          var valueKey = ':'.concat(index);
          var valueKey2 = ':'.concat(index, '2');
          ...
          if (filter.key.indexOf('.') > -1) {
            ...
            var splitKeysFormatted_1 = [];
            // We have to add a unique key for each splitKey
            splitKeys.forEach(function (splitKey, index2) {
              var fullSplitKey = ''.concat(key_1, 'split').concat(index2);
              if (validOptions.ExpressionAttributeNames) {
                validOptions.ExpressionAttributeNames[fullSplitKey] = splitKey;
              }
              splitKeysFormatted_1.push(fullSplitKey);
            });
            // Join the keys back together so its nested again
            key_1 = splitKeysFormatted_1.join('.');
          } else {
            validOptions.ExpressionAttributeNames[key_1] = filter.key;
          }
      ...
      var operation = options.filtersOperation || 'AND';
      validOptions.FilterExpression = filterExpressions_1.join(
        ' '.concat(operation, ' '),
      );
    }
          ...
          case 0:
            ...
            Object.keys(formattedData_1).forEach(function (key) {
              if (
                ...
                if (
                  formattedData_1[key] === null ||
                  formattedData_1[key] === ''
                ) {
                  removeExpressions_1.push('#'.concat(key));
                  delete ExpressionAttributeValues_1[':'.concat(key)];
                } else if (
                  formattedData_1.incrementValues.indexOf(key) === -1
                ) {
            ...
            if (removeExpressions_1.length > 0) {
              UpdateExpression += ''
                .concat(UpdateExpression ? ' ' : '', ' REMOVE ')
                .concat(removeExpressions_1.join(', '));
            }
            ...
            if (Object.keys(ExpressionAttributeValues_1).length === 0) {
  ...
  DynamoWrapper.prototype.batchGetAll = function (items, attributes) {
    return __awaiter(this, void 0, void 0, function () {
      var batchCommandInput,
        attempts,
        maxAttempts,
        timeBetweenEvents,
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            attemptUntilSuccessful = function (itemsToGet) {
              return __awaiter(_this, void 0, void 0, function () {
                var command, results, UnprocessedKeys;
                var _a;
                var _b, _c, _d;
                return __generator(this, function (_e) {
                  switch (_e.label) {
                    case 0:
                      attempts += 1;
                      command = new client_dynamodb_1.BatchGetItemCommand(
                        itemsToGet,
                      );
                      return [4 /*yield*/, this.mainIndex.send(command)];
                    case 1:
                      ...
                      UnprocessedKeys =
                        ((_c =
                          (_b = results.UnprocessedKeys) === null ||
                          _b === void 0
                            ? void 0
                            : _b[this.model.tableName]) === null ||
                        _c === void 0
                          ? void 0
                          : _c.Keys) || [];
                      if (
                        !(UnprocessedKeys.length > 0 && attempts < maxAttempts)
                      )
                        return [3 /*break*/, 4];
                      return [
                        4 /*yield*/,
                        (0, sleep_1.default)(timeBetweenEvents * attempts),
                      ];
                    case 2:
                      _e.sent();
                      return [
                    ...
                    case 4:
                      return [
                        2 /*return*/,
                        (_d = results.Responses) === null || _d === void 0
                          ? void 0
                          : _d[this.model.tableName],
                      ];
                  }
                });
              });
            };
            return [4 /*yield*/, attemptUntilSuccessful(batchCommandInput)];
          case 1:
            results = _b.sent();
            Items =
  ...
  DynamoWrapper.prototype.splitArrayIntoChunks = function (arr, chunkSize) {
    if (chunkSize === void 0) {
      chunkSize = 100;
    }
    var result = [];
  ...
  DynamoWrapper.prototype.batchGet = function (items, attributes) {
    return __awaiter(this, void 0, void 0, function () {
      var arrayOfItemArrays, finalResults_1, promises_1, arrayOfResultsArrays;
      var _this = this;
      return __generator(this, function (_a) {
        switch (_a.label) {
          case 0:
            if (!(items.length > 100)) return [3 /*break*/, 2];
            arrayOfItemArrays = this.splitArrayIntoChunks(items);
            finalResults_1 = [];
            promises_1 = [];
            arrayOfItemArrays.forEach(function (arrayOfItems) {
              promises_1.push(_this.batchGetAll(arrayOfItems, attributes));
            });
            return [4 /*yield*/, Promise.all(promises_1)];
          case 1:
            arrayOfResultsArrays = _a.sent();
            arrayOfResultsArrays.forEach(function (resultsArray) {
              finalResults_1 = __spreadArray(
                __spreadArray([], finalResults_1, true),
                resultsArray,
                true,
              );
            });
            return [2 /*return*/, finalResults_1];
          case 2:
            return [4 /*yield*/, this.batchGetAll(items, attributes)];
          case 3:
