...
When Lambda throws an unhandled exception, the AWS SDK returns `FunctionError` in the response.
The error payload contains `errorMessage`, `errorType`, and `stackTrace`.
...
When Lambda returns a non-200/202 status code, the function extracts error details from:
...
- Check the error message for "Invoke Lambda failed:" prefix to identify invokeLambda errors
- The function automatically constructs the Lambda function name from environment variables:
